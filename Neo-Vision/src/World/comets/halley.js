...
const halley_texture = textureLoader.load(
...
const semiMajorAxis = 3000; // Adjust for the size of the orbit (a)
const semiMinorAxis = 2500; // Adjust for the size of the orbit (b)
const zAxisScale = 1; // Adjust for the z-axis scale (c)
...
const focus = new THREE.Vector3(-200, 500, 40); // Earth positioned at (0,0,0)

// Create the asteroid mesh
function createhalley() {
  const halley = new THREE.Mesh(geometry, material);

  // Set initial position for the asteroid, start at one end of the ellipse
  halley.position.copy(new THREE.Vector3(semiMajorAxis - focusDistance, 0, 0)); // Start at (a - f, 0, 0)
...
function createhalleydirection(halley) {
  ...
  function animate() {
    ...
    halley.position.copy(newPosition);
